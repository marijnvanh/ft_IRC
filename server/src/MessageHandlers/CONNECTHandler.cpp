...
auto CONNECTHandler::SafeHandle(IMessage &message) -> void
{
	...
	{
		client->Push(GetErrorMessage(user->GetPrefix(), ERR_NOPRIVILEGES, "CONNECT"));
		return ;
	}
	...
	{
		client->Push(GetErrorMessage(user->GetPrefix(), ERR_NOSUCHSERVER, params[SERVER_NAME_PARAM]));
        return ;
    }
