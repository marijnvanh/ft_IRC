...
{
public:
    Message(std::shared_ptr<IRC::Mutex<IClient>> client, IRC::RawMessage message);
    ~Message();
    ...
    auto GetClient() const noexcept -> std::shared_ptr<IRC::Mutex<IClient>> override;

private:
    std::shared_ptr<IRC::Mutex<IClient>> client_;
    IRC::UUID uuid_;
