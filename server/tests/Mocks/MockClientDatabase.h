...
class MockClientDatabase : public IClientDatabase {
    ...
    MOCK_METHOD(void, RemoveUser, (const std::string &nickname), ());
    MOCK_METHOD(void, RemoveServer, (const std::string &server_name), ());
    MOCK_METHOD(std::shared_ptr<IClient>, GetClient, (IRC::UUID uuid), ());
    MOCK_METHOD(std::optional<std::shared_ptr<IClient>>, GetClient, (const std::string &nickname), ());
